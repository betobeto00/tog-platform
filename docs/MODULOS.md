...
| 1 | **Productor** | ✅ MVP v1 (`tog-admin`, migración 036, gating por licencia y permisos, tests) | Siembras (cultivo, área, cantidad sembrada, estado activa/cosechada), costos de campo por siembra y análisis de costos. Pendientes: estimación de cosecha y logística de acopio | Core |
| 2 | **Procesador** | ✅ MVP v1 (`tog-admin`, migraciones 049/050/051, gating por licencia y permisos, tests) | Cadenas de producción con pasos (entradas, costo unitario, tiempo estimado, margen) y **lotes** de transformación: al completarlos descuentan materia prima y suman producto terminado al stock. Pendientes: mermas y recetas/BOM avanzadas | Core + Productor (opcional) |
...
| 5 | **Postventa** | ✅ MVP v1 (`tog-admin`, migración 037, gating por licencia y permisos, tests) | Tickets de soporte con numeración `PV-000001` y estados abierto/en proceso/resuelto/cerrado, devoluciones que **reponen stock** o quedan como nota de crédito, garantías con resolución (reponer/reembolsar/rechazar). Pendientes: SLA y adjuntos en tickets | Core + Comercializador |
...
| 9 | **Almacenes** | ✅ MVP v1 (`tog-admin`, migraciones 029/043, gating por permisos, tests) | Múltiples depósitos, stock y movimientos por almacén, caja asociada al almacén | Core + Comercializador |
| 10 | **Hípico** | ✅ MVP v1 (`tog-admin`, migraciones 047/048, gating por licencia y permisos, tests) | Propietarios, caballos, carreras, inscripciones, resultados y venta de apuestas (con impresión de comprobante) | Core + Comercializador |
